...
```bash
npx --yes skills@latest add yearth/skills -g -a codex --skill brainstorming minimal-change task-handoff qa-gate-review -y
```

安装到多个 Agent：

```bash
npx --yes skills@latest add yearth/skills -g -a codex claude-code pi --skill brainstorming minimal-change task-handoff qa-gate-review -y
```
...
```bash
npx --yes skills@latest update brainstorming minimal-change task-handoff qa-gate-review -g -y
```
...
| [qa-gate-review](skills/qa-gate-review/SKILL.md) | 按风险验收真实执行路径；独立审查按需启用，UI 必须截图留证并交由用户人工审核。 |
